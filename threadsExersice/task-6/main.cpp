...
public:
    ...
    bool push(const T& val, std::chrono::steady_clock::duration timeout = std::chrono::seconds(0)) {
        ...
        if (_queue.size() == _limit && _limit)
        {
            if (timeout > std::chrono::seconds(0))
            {   
                bool noTimeout = _not_limit_cv.wait_for(l, timeout, [this] { return _queue.size() !=_limit; });
    ...
    bool pop(T& out, std::chrono::steady_clock::duration timeout = std::chrono::seconds(0)) {
        ...
        if (_queue.empty())
        {
            if (timeout > std::chrono::seconds(0))
            {
                bool noTimeout = _not_empty_cv.wait_for(l, timeout, [this] { return !_queue.empty(); });
